...
#include <candevice.h>
#include <configuration.h>	// needed mostly for the wheel circumference. Static methods.
#include <conversion.h>
#include <cmd_motctrl.h>

// Following are the IDs of the CAN-Devices:
// IOBoard:     101 for sending, 100 for receiving
// Right Motor: 201 for sending, 200 for receiving
// Left  Motor: 301 for sending, 300 for receiving
...
/// @class Drive
/// @brief The Drive-class represents the two motors, their brakes, odometers (=encoders) and temperature sensors.
...
class Drive : protected CanDevice
{
	public:
		Drive();
		~Drive();
		...
		/// @param left this argument will be set to the left advance in micrometers
		/// @param right this argument will be set to the right advance in micrometers
		///
		void getMotorAdvances(int &left, int &right);
		...
		///
		bool setEmergencyStop(const bool enable);
	...
	double debugPathLengthRight;
		...
		Logger* _logger;		///< Our logging object.
